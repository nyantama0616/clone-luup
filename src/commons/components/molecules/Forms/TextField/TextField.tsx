...
// TODO: フォーカス時の色を変えたい
function TextField({ label, required = false, placeholder="", className="", name, }: TextFieldProps) {
    ...
    return (
        <label className={_className}>
            ...
            <div className="col-span-3 flex items-center space-x-4">
                {requiredTag}
                <input type="text" placeholder={_placeholder} required={required} name={name} className="grow h-[56px] px-3 py-2 border border-gray-200 rounded-md placeholder-gray-200 focus:ring-red focus:border-red" />
            </div>
        </label>
    );
}
